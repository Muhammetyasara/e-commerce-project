    ...
    <header className="w-full">
      ...
      <div className="flex flex-col py-6 gap-10 lg:py-2 lg:flex-row lg:justify-between lg:px-52">
        <div className="flex justify-between lg:hidden">
          <Link to="/" className="text-lg font-semibold text-black lg:font-bold lg:text-xl">
            Bandage
          </Link>
        ...
        >
          ...
          <div className="lg:flex lg:flex-row gap-6 flex flex-col items-center">
            <Link to="/" className="text-lg font-medium text-gray-600">Home</Link>
                ...
                <div className="absolute top-full left-0 mt-2 w-80 rounded-md border bg-white shadow-lg z-50 p-4">
                  <div className="flex gap-8">
                    <div className="flex-1">
                      <h3 className="font-bold text-sm text-gray-800 mb-3 pb-2 border-b">Kadın</h3>
                    ...
                    <div className="flex-1">
                      <h3 className="font-bold text-sm text-gray-800 mb-3 pb-2 border-b">Erkek</h3>
                      <div className="flex flex-col gap-2">
            ...
            <Link to="/" className="text-lg font-medium text-gray-600">Blog</Link>
            ...
            <div className="relative">
              ...
              {isPagesOpen && (
                <div className="absolute top-full left-0 mt-2 w-40 rounded-md border bg-white shadow-lg z-50">
                  <Link to="/contact" className="block px-4 py-2 text-sm text-gray-600 hover:bg-gray-100" onClick={() => setIsPagesOpen(false)}>Contact</Link>
                  <Link to="/product" className="block px-4 py-2 text-sm text-gray-600 hover:bg-gray-100" onClick={() => setIsPagesOpen(false)}>Product</Link>
                  <Link to="/team" className="block px-4 py-2 text-sm text-gray-600 hover:bg-gray-100" onClick={() => setIsPagesOpen(false)}>Team</Link>
                  <Link to="/pricing" className="block px-4 py-2 text-sm text-gray-600 hover:bg-gray-100" onClick={() => setIsPagesOpen(false)}>Pricing</Link>
                  <Link to="/about" className="block px-4 py-2 text-sm text-gray-600 hover:bg-gray-100" onClick={() => setIsPagesOpen(false)}>About</Link>
                </div>
              )}
            </div>
          </div>
            ...
            {isUserLoggedIn ? (
              <div className="relative" ref={userMenuRef}>
                ...
                >
                  ...
                  <ChevronDown
                    size={16}
                    className={`text-gray-500 transition-transform hidden lg:block ${isUserMenuOpen ? "rotate-180" : ""}`}
                  />
                </button>

                {isUserMenuOpen && (
                  <div className="absolute top-full right-0 mt-2 w-48 rounded-md border bg-white shadow-lg z-50 py-1">
                    <Link
                      to="/orders"
              ...
              <div className="absolute top-full right-0 pt-3 min-w-[260px] max-w-[320px] z-[999] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition duration-200">
                <div className="bg-white border rounded shadow-lg p-3 flex flex-col">
                  <div className="flex justify-between items-center border-b pb-2 mb-2">
                    <span className="font-semibold text-sm">Sepetim ({cartItemCount} ürün)</span>
                  </div>
                  {cart.length === 0 ? (
                    ...
                    <div className="flex flex-col gap-3 max-h-60 overflow-auto">
                      {cart.map((item) => {
                        ...
                        const imgUrl = prod.images?.[0]?.url || prod.img || prod.image || "";
                        return (
                          <div key={prod.id} className="flex gap-3 items-start">
                            {imgUrl ? (
                              <img src={imgUrl} className="w-14 h-14 object-cover rounded" />
                            ) : (
                              <div className="w-14 h-14 bg-gray-200 rounded" />
                            )}
                            <div className="flex flex-col text-sm">
                              <span className="font-medium">{prod.name || prod.title}</span>
                  ...
                  <div className="flex gap-2 mt-3 pt-2 border-t">
                    ...
                    <button
                      ...
                      className="w-1/2 bg-sky-500 text-white py-2 rounded text-sm font-semibold hover:bg-sky-600"
                    >
                      Siparişi Tamamla
                    </button>
                  </div>
